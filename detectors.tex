...
The LIGO detectors completed the transition to their current \ac{aLIGO} design stage in 2015, and began their first observing run (\acs{O1}) on September 12 that year, making the first detection of a \ac{GW} from a \ac{BBH} merger on September 14~\citep{gw150914}.
...
We study detector noise in the frequency domain by computing the DARM signal \ac{ASD}, which is the square root of the \ac{PSD}.
Since the PSD represents the variance of the signal per frequency bin, it is expressed in DARM units squared per unit frequency, or m$^2$/Hz, so the ASD has units m/$\sqrt{\mathrm{Hz}}$.
Figure~\ref{fig:detectors-budget} is the \textit{noise budget} of the LHO and LLO detectors.
It shows the DARM ASD and estimates of all known noise sources affecting both LIGO detectors at the end of \ac{O3}, including those presented in this dissertation (scattered light and input beam jitter).
...
\textit{Scattered light noise} occurs when the laser light scatters from imperfections on an optic, like a test mass, and ends up hitting surfaces that are moving relative to the optic, like vacuum chamber walls.
...
In principle the symmetry of the interferometer arms should reject the effects of jitter, but defects in the test masses can break this symmetry, resulting in significant noise at the jitter frequencies, which are typically associated with mechanical resonances of optic mounts on the input laser table.
...
In Initial LIGO, permanent magnets were placed on the test masses themselves; in \ac{aLIGO} these were moved onto the test mass suspension systems instead.
The suspension system suppresses permanent magnet displacement $\propto f^{-2}$, or greater for the upper suspension stages, so it is only likely to dominate at low frequencies.
...
\textit{Length control} and \textit{alignment control} noise are associated with the control systems keeping the interferometer stable; \textit{laser intensity} and \textit{laser frequency} noise originate from the interferometer laser itself; \textit{Newtonian noise} comes from local gravitational field fluctuations caused by ground motion; \textit{residual gas noise} is caused by gas molecules in the vacuum chambers passing through the interferometer beam.
The noise investigations carried out through the duration of this PhD mainly covered noise effects associated with external vibrations (particularly scattered light and beam jitter) and magnetic fields.
