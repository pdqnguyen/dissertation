...
Long-duration \ac{ADI} injections are used to represent \acp{GW} produced by instabilities in the magnetically suspended torus around a rapidly spinning \ac{BH}, as described in Section~\ref{sec:gw-grb-long}~\citep{vanPutten_2001, vanPutten_2004}.
...
We run \xpip on each \ac{GRB} without autogating first, and perform an autogated rerun only if the closed box results suggest that autogating would improve the sensitivity of the search.
...
The Gaussianity measures, representing the ratios between the variance of the detector spectra to their means; high values correspond to high deviations from Gaussian behavior.
