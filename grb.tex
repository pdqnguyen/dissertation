...
\end{equation}
where each component of the vectors $\mathbf{d}$ and $\mathbf{n}$ is the output of a detector and its noise, respectively,
\begin{equation}
	\mathbf{h} = \left( \begin{array}{c} h_+ \\ h_{\times} \end{array} \right)
\end{equation}
represents the plus and cross polarizations of the GW signal, and $\mathbf{F} = [\mathbf{F_+}\,\,\mathbf{F_\times}]$ is the matrix whose rows represent the antenna response of each detector to the plus and cross polarizations of the GW signal.
The antenna response matrix is dependent on sky location.
...
\end{equation}
where $P^{\mathrm{GW}} \equiv \mathbf{F} (\mathbf{F}^{\dagger} \mathbf{F})^{-1} \mathbf{F} \mathbf{F}^{\dagger}$.

To identify signals, \xpip Fourier transforms the individual detector streams to produce time–frequency maps representing $\mathbf{d}$.
...
The time-frequency maps are combined over the detector network via \cref{eq:grb-esl} to form a single map of pixels representing the coherent energy.
Clusters are identified by selecting the 1\% of pixels with the highest coherent energy and grouping together adjacent selected pixels.
Each cluster is assigned an overall detection statistic which is the sum of the coherent energies of its pixels, and the cluster with the largest statistic is the best candidate for a GW detection.
...
Binary inspiral injections are chirp-like waveforms generated based on masses sampled from a Gaussian distribution centered at 1.4\,$\msol$, with a width of 0.2\,$\msol$ for an \ac{NS} in a \ac{BNS}, and with a width of 0.4\,$\msol$ for an \ac{NS} in an \ac{NSBH}.
...
The spin-down produces GWs following a downwards-sweeping signal, different from the chirp that would be seen if the binary was simply spiraling inwards.
