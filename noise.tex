...
\begin{figure}[h!]
	...
	\caption{
		The \ac{PEM} system layout at \ac{LLO} during \ac{O3}, as seen on the \ac{PEM} public website~\cite{PEM_website}.
		The path of the main interferometer laser is shown as a red line; core optics, such as the test masses, are represented by rectangles inside the vacuum chambers.
		The most major changes during \ac{aLIGO} have been made to the accelerometer locations and the addition of new magnetometers, e.g. in the electronics bays.}
	\label{fig:channels}
\end{figure}
...
\begin{figure}[h!]
	...
	\caption{
		Standard locations for vibration and magnetic injections at the \ac{LHO} corner station (left), Y end station (top right), and X end stations (bottom right).
...
\textit{Beating-shakers technique.}
...
\begin{figure}[h!]
	...
	\caption{
		Spectrograms of \ac{DARM} and various accelerometers near the input arm and beam splitter showing a beating-shakers injection used to localize the source of the 48\hyp Hz peak in \ac{DARM} at \ac{LHO} during the first half of \ac{O3}.}
	\label{fig:beats}
\end{figure}

\textit{Impulse injections}
...
\begin{figure}[h!]
	...
	\caption{
		Time series (left) and spectrograms (right) of a vibrational impulse injection produced at the output arm of the \ac{LHO} detector during an investigation corner station noise during \ac{O3}.}
	\label{fig:impulse}
\end{figure}


\subsubsection{Magnetic injections}\label{sec:magnetic-injections}

Improvements have also been made to the magnetic field injection equipment. In order to generate fields strong enough to couple into the \ac{GW} channel using the 1\,m magnetic field coils built during \ac{iLIGO}~\cite{Effler_2015}, we must focus the power of the coil into narrow bands and combs instead of injecting broadband signals. This was sufficient in \ac{iLIGO} when strong magnetic coupling occurred primarily through permanent magnets. However, due to the removal of permanent magnets from the test masses, coupling from those sources has decreased and cables and connectors have become the dominant coupling sites above about 80\,Hz, introducing more structure to the coupling functions and requiring stronger injections.
...
\begin{figure}[h!]
	...
	\caption{
		...
		The $5\times5$\hyp m coil allows for broadband injections at the same amplitudes that on the 1\hyp m coil required using comb injections.
		This tremendously improves coverage of potential magnetic coupling sources.}
	\label{fig:wallcoil}
\end{figure}
...
\subsection{Coupling functions}\label{sec:cf}
...
\begin{figure}
	...
	\caption{
		Example of a vibrational noise injection and measurement of a single-injection coupling function.
		The noise injected is a broadband (60-200\,Hz) acoustic signal produced near the input arm at \ac{LLO}.
		The top plot shows the displacement of an accelerometer in the \ac{PSL} room during background time (black) and injection time (orange).
		The middle plot shows the interferometer readout during background time (black) and injection time (orange).
		Estimated ambient levels for the accelerometer are also shown as dark blue dots, with upper limits shown as light blue crosses; they are produced from the single-injection coupling function in the bottom plot.}
	\label{fig:injection}
\end{figure}
...
\begin{figure}[h!]
	...
	\caption{
		Single-injection coupling functions (upper limits not shown) for the HAM5 Y-axis accelerometer for three different shaker injection locations (on top of HAM5, on top of HAM6, and on the HAM5 chamber door) show the typical spread in coupling that results from varying the injection location.
		Multiple injections at different frequency bands are shown for each source location.
		On average the coupling measured from different locations varies by a 1.4.}
...
\subsection{The {\fontfamily{pcr}\selectfont pemcoupling}\xspace package}
...
\subsection{Vibrational noise studies during O3}

\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{figures/ambient_vib.pdf}
	\caption{
		Ambient estimate of vibrational noise levels at \ac{LHO} (top) and \ac{LLO} (bottom).
		The \ac{DARM} background of each detector is shown in black, the \ac{aLIGO} sensitivity is shown in orange, and the noise estimates are shown in dark and light blue (for measurements and upper limits, respectively).}
...
\subsubsection{Input beam jitter}

\begin{figure}
	\centering
%	\includegraphics[width=\textwidth]{}
	\caption{Improvement in jitter coupling at \ac{LHO} (left) and \ac{LLO} (right) between the start and end of \ac{O3}.}
	\label{fig:jitter}
\end{figure}

\subsubsection{Scattered light at the HAM5/6 septum}

\subsubsection{Search for the source of a 48-Hz peak}

\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{figures/48Hz.png}
	\caption{\ac{LHO} \ac{DARM} spectrum before and after mitigation of the 48-Hz peak.}
	\label{fig:48hz}
\end{figure}

\subsection{Magnetic noise studies during O3}

\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{figures/ambient_mag.pdf}
	\caption{
		Ambient estimate of magnetic noise levels at \ac{LHO} (top) and \ac{LLO} (bottom).
		The \ac{DARM} background of each detector is shown in black, the \ac{aLIGO} sensitivity is shown in orange, and the noise estimates are shown in dark and light blue (for measurements and upper limits, respectively).}
...
\subsubsection{Fluctuations in magnetic coupling}

\begin{figure}
	\centering
%	\includegraphics[width=\textwidth]{}
	\caption{
		Ambient estimate of magnetic noise levels at \ac{LHO} on Mar \XX, 2019 (blue), \XX weeks before \ac{O3}, and on Apr \XX, 2019 (orange), \XX weeks into \ac{O3}.}
