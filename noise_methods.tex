...
Consider two sine waves of unit amplitude injected at frequencies $f_1$ and $f_2$ from two source locations.
For simplicity, we can consider that the sources lie along the $x$-axis.
Let $x_1$ be the location of source 1, $x_2$ the location of source 2, and $x$ the destination.
The individual signals are
\begin{align}
	y_1(t) &= \cos(k_1 (x - x_1) + 2 \pi f_1 t)\\
	y_2(t) &= \cos(k_2 (x - x_2) + 2 \pi f_2 t)
\end{align}
where $k_1 = 2 \pi f_1 / c_s$ and $k_2 = \pi f_2 / c_s$ with $c_s$ the speed of sound in the injection medium.
We observe the sum of these signals at their destination:
\begin{equation}
	\begin{split}
	  y_1(t) + y_2(t) = & 2 \cos \left\{ \frac{1}{2} \left[k_1 (x - x_1) + k_2 (x - x_2) + 2 \pi (f_1 + f_2) t \right] \right\}\\
		& \times \cos \left\{ \frac{1}{2} \left[k_1 (x - x_1) - k_2 (x - x_2) + 2 \pi (f_1 - f_2) t \right] \right\}
	\end{split}
\end{equation}
The first cosine factor is a wave with the average frequency $(f_1 + f_2) / 2$ and a phase determined by the distances $x_1$ and $x_2$.
The second factor represents the beat envelope, with a beat frequency $\Delta f = f_1 - f_2$.
The phase of this envelope at the destination is dependent on the locations of the sources:
\begin{align}
	\varphi & \equiv \frac{1}{2} \left[k_1 (x - x_1) - k_2 (x - x_2)\right] \\
					& = \frac{\pi}{c_s} \left[f_1 (x - x_1) - f_2 (x - x_2)\right]
\end{align}
Consider another destination at a different location $x'$.
We can write its envelope phase as
\begin{equation}
	\varphi' = \frac{\pi}{c_s} \left[ f_1 (x' - x_1) - f_2 (x' - x_2) \right]
\end{equation}
so the difference in the envelope phase between the two destinations is
\begin{equation}
	\delta \varphi = \varphi - \varphi' = \pi \frac{\Delta f}{c_s} (x - x').
\end{equation}
To distinguish two destinations, we need to observe phase differences on the scale of a half-period: $\delta \varphi \sim \pi$. For $\Delta f = 0.01\,\mathrm{Hz}$ and $c_s \simeq 3000\,\mathrm{m/s}$ for a shear wave in stainless steel, we can distinguish destinations of $\sim3\times10^{5}\,\mathrm{m}$ apart.
