...
\end{equation}
The amplitude modulations observed at two locations will be anti-phase if the path length difference for one location differs from that of the other by $\simeq 30$\,m for $f_1 = 100$\,Hz in stainless steel.
We can say that, for instance, the locations are more than a few meters apart if they observe beats arriving over a tenth of a beat period out of phase.
...
For each sensor, if the phase of this pattern is significantly offset from that of the DARM signal, then we reject the possibility that it is located close (within a few meters) to a coupling site.

Some sensors very far away from the coupling site can have an offset that is a multiple of a beat period, causing them to also line up with DARM.
This is not unlikely in the large observatory buildings, so based on one beating-shakers injection we cannot rule out these false positives.
...
A sensor within a few meters of a coupling site will still correlate with DARM, but the phase of far away sensors will change, allowing us to rule out a new subset of sensors with each unique injection configuration.
...
We can distinguish these arrival times because the propagation velocity along the chamber walls is only roughly 300\,m/s in our case. Using time series plots, the arrival time of the impulse in the \ac{GW} channel is compared to the arrival time of the impulse in multiple accelerometers.
