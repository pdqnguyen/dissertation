...
Although such expediency is not necessary for fixing malfunctioning hardware, it is important to have up-to-date channel status when validating recently detected \ac{GW} event candidates, as discussed in Section~\ref{sec:cf-vetting}.
...
Noise far above or below the reference is a sign of changes to hardware or infrastructure in the vicinity of the sensor, which may not adversely affect coverage of the area but could adversely affect nosie estimates as described in Section~\ref{sec:cf-vetting}.
...
As seen in Figure~\ref{fig:cf-cartoon1}, the sensor coupling function $C_j(f)$ is a good approximation to the true coupling $C(f)$ if the injection distance is much greater than the distance between the coupling site and sensor.

\begin{figure}[h]
	\includegraphics[width=\textwidth]{figures/cf-cartoon1.pdf}
	\caption{
		Diagram of a coupling function measurement.}
	\label{fig:cf-cartoon1}
\end{figure}
...
This is illustrated in Figure~\ref{fig:cf-cartoon2}:
in the nearest-injection approximation the coupling of sensor $j$, $C_j(f)$, is measured using the observation of the nearest injection, in this case $x_{ij}(f)$, while $C_{j'}(f)$ is measured using $x_{i'j'}(f)$.).
...
\begin{figure}[h]
	\includegraphics[width=\textwidth]{figures/cf-cartoon2.pdf}
	\caption{
		Expanding Figure~\ref{fig:cf-cartoon2} to more than one sensor and coupling site.
	}
	\label{fig:cf-cartoon2}
\end{figure}
...
The coupling function as approximated in \cref{eq:ccf} is used for comparing coupling between different sensor locations and producing estimates of interferometer noise levels, e.g. as part of event validation (see Section~\ref{sec:noise-vetting}).
...
That said, more precise noise estimates may become important for quantifying the impact of environmental transients on \ac{GW} event candidates, as discussed in Section~\ref{sec:noise-vetting}.
...
\section{Noise estimation with coupling functions}\label{sec:cf-vetting}

{\color{red}
This is where I'll talk about the methodology for vetting, including O4 pemcheck work (still in progress), as well as ligocam considerations.}
