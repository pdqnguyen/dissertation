...
\section{Evaluation of coupling functions}\label{sec:noise-tests}
...
As we know systematic uncertainties result in coupling measurements to be log-normally distributed (see Section~\ref{sec:hardware-limitations}), we can describe the true coupling with a log-normal distribution with mean $\ln C(f_k)$ and standard deviation $\ln 2$ (corresponding to a factor two uncertainty).
...
\begin{figure}[h!]
	\includegraphics[width=\textwidth]{figures/cf-vetting-spectrogram1.png}
	\includegraphics[width=\textwidth]{figures/cf-vetting-spectrogram2.png}
	\caption
	...
	\label{fig:cf-vetting-spectrogram}
\end{figure}

Figure~\ref{fig:cf-vetting-spectrogram} provides an example of a probability spectrogram output by \code{pemcheck} for an \ac{O3} \ac{GW} event candidate, S190510g.
...
\begin{figure}[h!]
	\includegraphics[width=\textwidth]{figures/cf-vetting-tuning1.png}
	\includegraphics[width=\textwidth]{figures/cf-vetting-tuning2.png}
	\caption
	...
	\label{fig:cf-vetting-tuning}
\end{figure}

Figure~\ref{fig:cf-vetting-tuning} shows an egregious case of projected noise for an accelerometer on the squeezed light optics table overestimating the strain background amplitude.
...
Since coupling functions are only intended to be used for noise sources distant from the sensors, projections from sensors placed close to each other ($\leqslant$ 1\,m) should be highly correlated if the input noise is environmental and not local to the individual sensors.
To incorporate this knowledge, \code{pemcheck} groups together magnetometers each electronics rooms; their probability spectrograms are stacked and a pixel-wise minimum is computed to generate a combined group spectrogram.
