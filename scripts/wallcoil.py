...
def plot_wallcoil():
    ...
    plt.subplots_adjust(left=0.16, bottom=0.15, right=0.99, top=0.9)
    ax.plot(broadband_freqs, np.maximum.reduce(broadband_injs), color='#015d8e', label=r'5$\times$5\,m coil')
    ax.plot(comb_freqs, np.maximum.reduce(comb_injs), color='#85c0f9', label='1\,m coil')
    ax.plot(comb_freqs, comb_bg, 'k-')
    # ax.plot(broadband_freqs, broadband_bg, 'k-')
    # ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlabel('Frequency [Hz]')
    ax.set_ylabel(r'DARM $\left[\mathrm{m}/\sqrt{\mathrm{Hz}}\right]$')
    ax.set_xlim(xlim)
    ax.set_ylim(ylim)
    ax.grid(True, which='major', axis='both')
    ax.legend(loc='upper right')
    # ax[0].loglog(comb_freqs, np.maximum.reduce(comb_injs), color='#f5793a')
    # ax[0].plot(comb_freqs, comb_bg, 'k-')
    # ax[1].loglog(broadband_freqs, np.maximum.reduce(broadband_injs), color='#f5793a')
    # ax[1].plot(broadband_freqs, broadband_bg, 'k-')
    # ax[0].set_xlabel('Frequency [Hz]')
    # ax[1].set_xlabel('Frequency [Hz]')
    # ax[0].set_ylabel(r'DARM $\left[\mathrm{m}/\sqrt{\mathrm{Hz}}\right]$')
    # ax[0].set_xlim(xlim)
    # ax[0].set_ylim(ylim)
    # ax[1].set_xlim(xlim)
    # ax[1].set_ylim(ylim)
    # ax[0].set_title('Comb injection')
    # ax[1].set_title('Broadband injection')
